...
The offline store is **always** updated. Write ordering differs by operation:

- **Mutations of existing entities are offline-first** — the local blob is written first (synchronously, stamped with the current time), then the server: `updateNote`/`updateListItem`, `removeNote`/`removeListItem`, `restoreNote`/`restoreListItem`, `completeNote`, `setNotesOrder`/`setListItemsOrder`. This keeps the local blob the up-to-date source of truth the instant the user acts: a focus/`online` re-sync that races the change reconciles by `updated` timestamp (last-write-wins) and keeps the fresh local value, instead of clobbering a change still in flight to the server (which made typed text briefly "disappear" until the next refresh). It also means a non-network server error no longer loses the local write.
- **Creates are the deliberate exception (`addNote`/`addListItem`) — online-first.** The server-assigned id must be stamped onto the offline record atomically; writing locally with a synthetic `offline-*` id first would risk a focus re-sync racing in before the id is promoted and creating the entity on the server a second time (duplicate). Offline durability for creates is still covered by the `tryOnlineCall` fallback: on a network failure the offline write proceeds with an `offline-*` id, which `SyncService` promotes to the server id on the next merge.
- **`updateProfile` is online-only** (the photo `File` can't be queued in LocalStorage): the facade throws when offline, the dialog blocks saving with a hint, and after a successful server response the confirmed user data (including the server-side avatar path) is written into the offline blob via `OfflineApiService.setUser`.

When changing this, keep new mutations of existing entities offline-first and creates online-first. Entities created offline get string ids prefixed `offline-<timestamp>` and are reconciled later.
...
`SyncService.handleApplicationUpdate(isUpdating)` is the entry point that coordinates a re-sync: it no-ops when offline or on the sign route, sets `globalStore.isUpdating`, runs `InitService.initApplication(...)`, and always resets `isUpdating` / `isSocketErrorOnce` in a `finally`. It's called from the window `online` event and from `HealthService.check` when connectivity is regained. The `globalStore.isUpdating` flag (the "Updating…" toast) is **owned by this visible-refresh trigger, not by `synchronizeOfflineData`** — `synchronizeOfflineData` only clears it defensively in its `finally`. That keeps the cold-start reconcile (see boot sequence) silent while focus/online re-syncs still surface the toast.
...
2. `boot/init.ts` — instantiates `ApiService`, wires `showError`, registers all `src/components/**/*.vue` globally (via `import.meta.globEager`, so component tags don't need explicit imports), also registers `zhyswan-vuedraggable` as the global `<Draggable>` component, starts `SocketIOService`, then calls `InitService.coldStart()`. Cold start renders the cached offline blob **immediately and synchronously** (`hydrateFromOffline`) — first paint shows real data, never skeletons, and without the "Updating…" toast — then refreshes from the server in the background via `initApplication(data, true)` (which seeds `TypesService` / `StatusesService` / `NotesService` and merges offline edits). On a fast failure (server down / connection refused) it drops to the offline icon at once; if the server instead stays silent, the `OFFLINE_ICON_MS` (4s) timer does so without waiting out `getConfig`'s own 10s timeout (`CONFIG_REQUEST_TIMEOUT_MS`). Both paths funnel through `goOfflineOrShowError`. Skeletons appear only when there is no cached blob to show — then that same fallback renders the connection-error screen instead.
...
- **Pug templates are type-checked** via `@vue/language-plugin-pug` (declared in `tsconfig.json` → `vueCompilerOptions.plugins`) — in the editor, in CLI `vue-tsc`, and in the build (`vite-plugin-checker` fails the build on template errors; verified). The toolchain is pinned as a set: `typescript` 5.5 + `vue-tsc` 3.3.x + the pug plugin **at the same version as the VSCode Vue extension (3.3.x)** + `vite-plugin-checker` 0.10.x (the last line supporting both vite 2 and vue-tsc 3) + `@typescript-eslint` 7.x. If the plugin's version ever drifts from the extension's, the editor silently stops seeing pug templates again: imports used only in templates get marked unused (TS hint 6192) and editor save-actions may delete them — that's exactly how icon imports once vanished. As a safety net, the project `.vscode/settings.json` pins `"source.organizeImports": "never"` for `[vue]` (overriding any global setting) — keep it. Note `vue-tsc` 3 is laxer than 1.8 about component prop *types* in templates unless `strictTemplates` is enabled; unknown identifiers are still caught.
