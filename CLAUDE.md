...
- No test runner is configured (`npm test` is a no-op).
...
A `?is-watch=1` query parameter puts the app into "watch mode" (`globalStore.isWatchMode`), which suppresses focus-driven re-syncs in `boot/init.ts`. Keep this guard in mind when adding new auto-refresh behavior.
